...
namespace BoingKit
{
  public class BoingBones : BoingReactor
  {
    ...
    public class Bone
    {
      ...
      internal Quaternion                    BlendedRotationLs;
      ...
      internal bool                          isFreezeRotationPosition;
      internal bool                          isClampYRotation;
      internal bool                          tempFreezePositionRotation;
      internal float                         timeToFreeze = 0.35f;
    ...
    private List<Bone> alreadyBoneList = new List<Bone>();
    bool               isSetTempFreezeBones;
    public void RescanBoneChains()
    {
      ...
      {
        isSetTempFreezeBones = false;
        var  chain            = BoneChains[iChain];
        
        /*//Akash ----- Logic Of Rescan Bones Limits 
        ...
        while (boneQueue.Count > 0)
        {
          ...
          for (int iChild = 0, numChildren = boneTransform.childCount; iChild < numChildren; ++iChild)
          {
            ...
            if (chain.Exclusion.Contains(childTransform) /*|| childTransform.GetComponent<NotABoingBone>()*/)
              continue;
          ...
          //If Already Bone Exist At this index then add from existing List
          if (alreadyBoneList.Count > 0 && bonesCounter < alreadyBoneList.Count && alreadyBoneList[bonesCounter].Transform == boneTransform)
          {
            aBone.Add(alreadyBoneList[bonesCounter]);
            if (numChildrenQueued > 0)
              alreadyBoneList[bonesCounter].ChildIndices = aChildIndex;
            bonesCounter++;
            isSetTempFreezeBones = true;
            continue;
          }

          chain.MaxLengthFromRoot = Mathf.Max(entry.LengthFromRoot, chain.MaxLengthFromRoot);
          boneTransform.rotation  = boneTransform.parent.rotation;
          // boneTransform.rotation  = boneTransform.rotation;
          var bone = new Bone(boneTransform, entry.ParentIndex, entry.LengthFromRoot);
          if (numChildrenQueued > 0)
            bone.ChildIndices = aChildIndex;

          // var boneFreezeCheck = bone.Transform.GetComponent<FreezeBone>();
          // bone.isFreezeRotationPosition = boneFreezeCheck && boneFreezeCheck.freezeRotationAndPosition;
          // bone.isClampYRotation         = boneFreezeCheck && boneFreezeCheck.clampRotation;

          if (isSetTempFreezeBones)
            bone.tempFreezePositionRotation = true;

          aBone.Add(bone);
          alreadyBoneList.Add(bone);
        }
    ...
    //Remove all bones form List After boneTransform
    public void RemoveBonesAfterTransformBone(Transform boneTransform)
    {
      var boneIndex     = 0;
      var isRemoveBones = false;
      var loopCount     = alreadyBoneList.Count;
      for (int i = 0; i < loopCount; i++)
      {
        if (i < alreadyBoneList.Count && alreadyBoneList[i].Transform == boneTransform && !isRemoveBones)
        {
          boneIndex     = i;
          isRemoveBones = true;
        }

        if (i > boneIndex && isRemoveBones)
        {
          alreadyBoneList.RemoveAt(boneIndex + 1);
        }
      }
    }
